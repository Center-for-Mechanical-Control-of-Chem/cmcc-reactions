...
template_1 = "[R][C@]1([X])[C@@H]2C=C[C@@H](C2)[C@@H]1[R’]"
template_2 = "[R][C@H]1[C@]([X])([C@@H]2C=C[C@H]1C2)[R’]"
...
for idx, (R, X, R_prime) in enumerate(combinations):
    ...
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2 + atom_indices[idx][1] + 4
    idx3, idx4 = atom_indices[idx][0] + 2, atom_indices[idx][0] + 2 + atom_indices[idx][1] + 1
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
...
for i in range(0,2):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction5/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction5/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction5/'+str(i)+'/'+str(j)+'/isomer.txt','w')
