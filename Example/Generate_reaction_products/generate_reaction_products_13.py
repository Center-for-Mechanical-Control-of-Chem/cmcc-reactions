...
template_1 = "[R]C1([R'])C(O2)(C3=CC=CC=C3)C(C=CC=C4)=C4C2(C5=CC=CC=C5)C1"
...
for idx, (R, R_prime) in enumerate(combinations):
    ...
    idx3, idx4 = atom_indices[idx][0] + 1 + atom_indices[idx][1] + 15, atom_indices[idx][0] + 1 + atom_indices[idx][1] + 22
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction13/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction13/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction13/'+str(i)+'/'+str(j)+'/isomer.txt','w')
