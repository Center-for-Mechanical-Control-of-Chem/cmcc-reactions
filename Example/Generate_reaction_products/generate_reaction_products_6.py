...
def generate_smiles(template, R, X, R_prime, B):
    smiles = template.replace("[R]", R).replace("[X]", X).replace("[R’]", R_prime).replace("[B]", B)
    return smiles
...
B_list = ["S(=O)(C8=CC=CC=C8)=O", "CN", "C(=O)N", "C9=CC=C(F)C=C9"]


template_1 = "[R][C@]1([X])[C@@H]2C=C[C@@H](C2)[C@]1([R’])[B]"



# Generate combinations of R, X, and R'
combinations = list(itertools.product(R_list, X_list, R_prime_list, B_list))
atom_indices = list(itertools.product(R_heavy_atom_indices, R_heavy_atom_indices, R_heavy_atom_indices, R_heavy_atom_indices))

All_smiles = [] 
Indices = []
for idx, (R, X, R_prime, B) in enumerate(combinations):
    smiles = generate_smiles(template_1, R, X, R_prime, B)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 1 + atom_indices[idx][1] + 1
    idx3, idx4 = atom_indices[idx][0] + 1 + atom_indices[idx][1] + 4 , atom_indices[idx][0] + 1 + atom_indices[idx][1] + 6
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction6/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction6/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction6/'+str(i)+'/'+str(j)+'/isomer.txt','w')
