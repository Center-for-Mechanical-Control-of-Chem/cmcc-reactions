...
template_1 = "[R]C1([R'])[C@@H](O2)C(C(OC)=O)=C(C(OC)=O)[C@@H]2C1"
...
for idx, (R, R_prime) in enumerate(combinations):
    ...
    idx3, idx4 = atom_indices[idx][0] + 1 + atom_indices[idx][1] + 13, atom_indices[idx][0] + 1 + atom_indices[idx][1] + 14
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction14/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction14/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction14/'+str(i)+'/'+str(j)+'/isomer.txt','w')
