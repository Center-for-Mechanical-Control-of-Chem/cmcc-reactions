...
def generate_smiles(template, R, X, R_prime):
    smiles = template.replace("[R]", R).replace("[X]", X).replace("[R’]", R_prime)
    return smiles

def gen_conformers(mol, numConfs=50, maxAttempts=1000, pruneRmsThresh=0.1, useExpTorsionAnglePrefs=True, useBasicKnowledge=True, enforceChirality=True):
...
template_1 = "[R][C@@H]1[C@@H]([X])C[C@H]([R’])C=C1"
template_2 = "[R][C@H]1[C@@H]([X])C[C@@H]([R’])C=C1"
...
for i in range(52,100):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction1/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction1/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction1/'+str(i)+'/'+str(j)+'/isomer.txt','w')
