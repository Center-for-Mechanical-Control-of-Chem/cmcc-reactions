...
template_1 = "[R][C@H]1[C@@H]([X])CNC=C1"
template_2 = "[R][C@@H]1[C@H]([X])CNC=C1"
template_3 = "[R][C@@H]1[C@@H]([X])CNC=C1"
template_4 = "[R][C@H]1[C@H]([X])CNC=C1"
template_5 = "[R][C@@H]1C[C@H]([X])NC=C1"
template_6 = "[R][C@H]1C[C@@H]([X])NC=C1"
template_7 = "[R][C@@H]1C[C@@H]([X])NC=C1"
template_8 = "[R][C@H]1C[C@H]([X])NC=C1"
...
for idx, (R, X) in enumerate(combinations):
    ...
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2
    idx3, idx4 = atom_indices[idx][0] + 2 + atom_indices[idx][1] + 1, atom_indices[idx][0] + 2 + atom_indices[idx][1] + 2
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, X) in enumerate(combinations):
    smiles = generate_smiles(template_2, R, X)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2 
    idx3, idx4 = atom_indices[idx][0] + 2 + atom_indices[idx][1] + 1, atom_indices[idx][0] + 2 + atom_indices[idx][1] + 2
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, X) in enumerate(combinations):
    smiles = generate_smiles(template_3, R, X)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2 
    idx3, idx4 = atom_indices[idx][0] + 2 + atom_indices[idx][1] + 1, atom_indices[idx][0] + 2 + atom_indices[idx][1] + 2
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, X) in enumerate(combinations):
    smiles = generate_smiles(template_4, R, X)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2 
    idx3, idx4 = atom_indices[idx][0] + 2 + atom_indices[idx][1] + 1, atom_indices[idx][0] + 2 + atom_indices[idx][1] + 2
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, X) in enumerate(combinations):
    smiles = generate_smiles(template_5, R, X)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2 
    idx3, idx4 = atom_indices[idx][0] + 3, atom_indices[idx][0] + 3 + atom_indices[idx][1] + 1
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, X) in enumerate(combinations):
    smiles = generate_smiles(template_6, R, X)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2
    idx3, idx4 = atom_indices[idx][0] + 3, atom_indices[idx][0] + 3 + atom_indices[idx][1] + 1
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, X) in enumerate(combinations):
    smiles = generate_smiles(template_7, R, X)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2
    idx3, idx4 = atom_indices[idx][0] + 3, atom_indices[idx][0] + 3 + atom_indices[idx][1] + 1
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, X) in enumerate(combinations):
    smiles = generate_smiles(template_8, R, X)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 2
    idx3, idx4 = atom_indices[idx][0] + 3, atom_indices[idx][0] + 3 + atom_indices[idx][1] + 1
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction9/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction9/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction9/'+str(i)+'/'+str(j)+'/isomer.txt','w')
