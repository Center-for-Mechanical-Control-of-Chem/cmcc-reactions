...
template_1 = "[R]C1(C)[C@@H]2C(C=CC=C3)=C3[C@H](C1)C=C2"
template_2 = "[R]C1(C)[C@H]2C(C=CC=C3)=C3[C@@H](C1)C=C2"
...
for idx in range(len(R_list)):
    ...
    idx3, idx4 = R_heavy_atom_indices[idx] + 1 + 1 + 8, R_heavy_atom_indices[idx] + 1 + 1 + 9
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx in range(len(R_list)):
    smiles = generate_smiles(template_2, R_list[idx])
    idx1, idx2 = R_heavy_atom_indices[idx] + 1, R_heavy_atom_indices[idx] + 1 + 1 + 1
    idx3, idx4 = R_heavy_atom_indices[idx] + 1 + 1 + 8, R_heavy_atom_indices[idx] + 1 + 1 + 9
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction15/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction15/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction15/'+str(i)+'/'+str(j)+'/isomer.txt','w')
