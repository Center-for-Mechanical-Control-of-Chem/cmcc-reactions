...
template_1 = "[R][C@@H]1[C@@H]2C=C[C@H]([C@H]1[X])C2"
template_2 = "[X][C@@H]1[C@@H]2C=C[C@H]([C@H]1[R])C2"
...
for idx, (R, X) in enumerate(combinations):
    ...
    idx3, idx4 = atom_indices[idx][0] + 1 + 4, atom_indices[idx][0] + 1 + 4 + 1
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction3/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction3/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction3/'+str(i)+'/'+str(j)+'/isomer.txt','w')
