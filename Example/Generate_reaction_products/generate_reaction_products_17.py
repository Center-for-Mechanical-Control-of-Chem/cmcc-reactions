...
template_1 = "[R]C1(C)[C@@H](O2)C(C(OC)=O)=C(C(OC)=O)[C@@H]2C1"
...
for idx in range(len(R_list)):
    ...
    idx3, idx4 = R_heavy_atom_indices[idx] + 1 + 1 + 13, R_heavy_atom_indices[idx] + 1 + 1 + 14
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction17/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction17/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction17/'+str(i)+'/'+str(j)+'/isomer.txt','w')
