...
template_1 = "[R][C@@H]1[C@@H]2C=C[C@@H](C2)[C@@H]1[X]"
template_2 = "[R][C@H]1[C@@H]2C=C[C@@H](C2)[C@H]1[X]"
...
for idx, (R, X) in enumerate(combinations):
    ...
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 1 + 1
    idx3, idx4 = atom_indices[idx][0] + 5, atom_indices[idx][0] + 7
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction4/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction4/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction4/'+str(i)+'/'+str(j)+'/isomer.txt','w')
