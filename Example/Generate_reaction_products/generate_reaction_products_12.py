...
def generate_smiles(template, R, R_prime):
    smiles = template.replace("[R]", R).replace("[R']", R_prime)
    return smiles
...
template_1 = "[R]C1([R'])[C@@H]2C(C=CC=C3)=C3[C@H](C1)C=C2"
template_2 = "[R]C1([R'])[C@H]2C(C=CC=C3)=C3[C@@H](C1)C=C2"


# Generate combinations of R, X, and R'
combinations = list(itertools.product(R_list, R_prime_list))
atom_indices = list(itertools.product(R_heavy_atom_indices, R_heavy_atom_indices))

All_smiles = [] 
Indices = []
for idx, (R, R_prime) in enumerate(combinations):
    smiles = generate_smiles(template_1, R, R_prime)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 1 + atom_indices[idx][1] + 1
    idx3, idx4 = atom_indices[idx][0] + 1 + atom_indices[idx][1] + 8, atom_indices[idx][0] + 1 + atom_indices[idx][1] + 9
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
for idx, (R, R_prime) in enumerate(combinations):
    smiles = generate_smiles(template_2, R, R_prime)
    idx1, idx2 = atom_indices[idx][0] + 1, atom_indices[idx][0] + 1 + atom_indices[idx][1] + 1
    idx3, idx4 = atom_indices[idx][0] + 1 + atom_indices[idx][1] + 8, atom_indices[idx][0] + 1 + atom_indices[idx][1] + 9
    All_smiles.append(smiles)
    Indices.append([[idx1,idx2],[idx3,idx4]])
...
for i in range(len(uniq_indices)):
    ...
    for j in top_indices:
        ...
        try:
            os.mkdir('Reaction12/'+str(i)+'/'+str(j))
        except:
            pass

        write('Reaction12/'+str(i)+'/'+str(j)+'/conf.xyz', conformer_tmp, format='xyz')

        f = open('Reaction12/'+str(i)+'/'+str(j)+'/isomer.txt','w')
